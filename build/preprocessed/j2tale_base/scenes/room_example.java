...
public class room_example extends RoomBase {

    // Устанавливаем радиус локации
    private int RenderRadiusX = (width / 20), RenderRadiusY = (height / 20) + 2;
    // private int RenderRadiusX = 5, RenderRadiusY = 5;

    public room_example(Midlet midlet) {
        super(midlet);

        // Импорт локаций
        loadMapLayers("/maps/gflowers.png", 20, 20);
        if (mapLayers != null) {
        ...
        // Импорт карты коллизий
        collisions = new TilesetMap("/maps/ruins1_collisions.csv", "/images/tiles/ruins.png", 20, 20);

        // Создание коллизии карты
        player.clearCollisionBoxes();
        if (collisions != null) {
            collisions.generateCollisions(player, new int[]{0});
        }

        // Установка размера карты, только если слой и данные карты не пустые
        if (mapLayers != null && mapLayers[0] != null) {
            int mapWidth = mapLayers[0].getMapWidth();
            int mapHeight = mapLayers[0].getMapHeight();
            if (mapWidth > 0 && mapHeight > 0) {
                player.setMapSize(mapWidth, mapHeight);
            }
        }
    }
